...
def forgot_password_view(request):
    ...
    if request.method == "POST":
        ...
        email = request.POST.get('emai', '').strip().lower()
        ...
        try:
            ...
            request.session['forgot_otp_created_at'] = timezone.now().isoformat()
...
def verify_forgot_otp_view(request):
    ...
    # if request.session.get('forgot_verified'):
    #     return redirect('reset_password')
        
    user_id = request.session.get('forgot_user_id')
    otp_session = request.session.get('forgot_otp')
    created_at = request.session.get('forgot_otp_created_at')

    if not user_id or not otp_session or not created_at:
        messages.error(request, 'session expire. Please try again.')
        return redirect('forgot_password')
